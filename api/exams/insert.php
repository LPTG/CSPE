...
if (empty($payload->examName) || empty($payload->examGroup) ||
    empty($payload->passFailPct) || empty($payload->author)) {
...
// Process SQL
$stmt = $pdo->prepare('INSERT INTO exams (examName, examGroup, passFailPct, author) 
                       VALUES (?, ?, ?, ?)');
$stmt->execute([$payload->examName, $payload->examGroup, $payload->passFailPct, $payload->author]);
$id = $pdo->lastInsertId();
$data = $stmt->rowCount(); 
