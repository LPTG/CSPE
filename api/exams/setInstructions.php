...
// Process SQL
$stmt = $pdo->prepare('UPDATE instructions SET instructions = ?');
$stmt->execute([$payload->instructions]);
$data = $stmt->rowCount();

if ($data) {
    http_response_code(200);
    echo '{"message" : "Instructions updated"}';
} else {
    http_response_code(200);
    echo '{"message" : "Instructions not updated." }';
}
?>
