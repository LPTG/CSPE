...
if (empty($payload->id) || empty($payload->examID)) {
...
// Process SQL
$stmt = $pdo->prepare('select (select count(*) from results where answered=correctAnswer and id = ? and examID = ?)
                    / (select count(*) from examContent where examId = ?) as score');
$stmt->execute([$payload->id, $payload->examID, $payload->examID]);
$data = $stmt->fetchAll(); 
