...
if (empty($payload->examID) || empty($payload->questionID) ) {
...
// Process SQL
$stmt = $pdo->prepare('DELETE FROM examContent WHERE examID = ? AND questionID = ?');
$stmt->execute([$payload->examID,$payload->questionID]);
$data = $stmt->rowCount(); 
