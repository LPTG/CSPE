...
// Process SQL
$stmt = $pdo->prepare('update users set examEndTime = IF (examEndTime IS NULL, NOW(), examEndTime) where id = ?');
...
if ($data) {
    ...
    echo '{"message" : "Exam end time set" }';
} else {
    http_response_code(200);
    echo '{"message" : "Unable to set exam end time" }';
}
