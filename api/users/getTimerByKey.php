...
// Process SQL
$stmt = $pdo->prepare('SELECT examTimelimit, examStartTime, date_add(examStartTime, INTERVAL examTimeLimit MINUTE) as examEndTime, now() as currentTime FROM users WHERE id = ?');
$stmt->execute([$key]);
$data = $stmt->fetch(); 
